...
- `BASE_URL` defaults from `APP_ORIGIN` so links generated by Rails use the public Cloudron URL.
...
The entrypoint prepares the primary schema, seeds the secondary Solid Queue/Cache/Cable schemas into Cloudron’s single MySQL database when needed, and then runs:
...
The entrypoint maps these values to Fizzy’s standard `SMTP_*` variables, so the upstream production configuration enables SMTP automatically.
...
- `JOB_CONCURRENCY=1` keeps the default Solid Queue process count suitable for a 2 GB Cloudron container; override it for larger installations.
...
The change takes effect on the next request. Set it back to `true` to re-enable public signups.
...
The publish automation lives in [`../.github/workflows/publish-image.yml`](../.github/workflows/publish-image.yml). The fork’s OSS CI validates the underlying application code; Basecamp’s private SaaS job is intentionally not run here.
